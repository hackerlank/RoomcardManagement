 ...
 */
class TransferMessage extends BaseMessage {
    ...
    public readTransfer(msg: any) {
        ...
        if (msg.x_uid) {
            var follow: FollowVo = this.gameManager.msgManager.follow.followMap[msg.x_uid];
     ...
     * 转账详情列表
     */
    public sendTransferList(uid: string = "", p: number = 1, n: number = 20) {
        var data: any = {};
        data.s = core.sessionid;
        data.p = p;
        data.n = n;
     ...
     * @param msg
     */
    public readTransferList(msg: any) {
        if (msg.code != 0)return;
